...
typedef struct
{
    ...
    uint16_t   nState, m_nStreamState;
	float		m_nPlayVolume;
    short		channel;
    V3XA_HANDLE	sample;
}DS_stream;
...
static void ChannelOpen(int gain, int channel)
{
    if (!channel)
		channel = 16;
    RLX.Audio.ChannelToMix = (uint8_t)channel;
    UNUSED(gain);
}
...
V3XA_STREAM StreamInitialize(int sampleFormat, int freq, size_t size)
{
    ...
    pHandle->sample.sampleFormat = (uint16_t)sampleFormat;
    ...
    {
        pHandle->nState |= 1;
        pHandle->m_nStreamState = (uint16_t)precache;

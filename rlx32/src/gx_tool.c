...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  void IMG_stretch(uint8_t *old_buf, uint8_t *new_buf, int old_ly, int new_ly, int old_lx, int new_lx, int bytes)
*
* DESCRIPTION :
*
*/
void IMG_stretch(uint8_t *old_buf, uint8_t *new_buf, int old_ly, int new_ly, int old_lx, int new_lx, int bytes)
{
    uint8_t *v=new_buf, *w;
    uint16_t *v0 = (uint16_t*)new_buf, *old_buf0=(uint16_t*)old_buf, *w0;
    uint32_t  *v1 = (uint32_t*)new_buf, *old_buf1=(uint32_t*)old_buf, *w1;
    rgb24_t *v2 = (    rgb24_t*)new_buf, *old_buf2=(    rgb24_t*)old_buf, *w2;
    // uint32_t *v1 = (uint16_t*)new_buf, *old_buf1=(uint16_t*)old_buf, *w1;
    int32_t i, j;
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  void IMG_pan(uint8_t *old_buf, uint8_t *new_buf, int old_ly, int new_ly, int old_lx, int new_lx)
*
* DESCRIPTION :   Modify the canvas size of a buffer
*
*/
void IMG_pan(uint8_t *old_buf, uint8_t *new_buf, int old_ly, int new_ly, int old_lx, int new_lx, int bytes)
{
    uint8_t *v=new_buf, *w=old_buf;
    int32_t i, j;
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  void IMG_rotate_image(uint8_t *old, uint8_t *tmp, int lx, int ly)
*
* DESCRIPTION :
*
*/
void IMG_rotate_image(uint8_t *old, uint8_t *tmp, int lx, int ly)
{
    uint8_t *v=old, *w=tmp;
    int i, j;
    for (i=lx;i!=0;i--, old++)
...
{
    uint8_t *v=GX.View.lpBackBuffer+(yy*GX.View.lPitch)+xx*GX.View.BytePerPixel, *w=capt->data;
    uint32_t cx, lx=GX.View.BytePerPixel*capt->LX;
...
void IMG_StretchBilinear(GXSPRITE *_new, GXSPRITE *_old, int bp, int strides)
{
    ...
    if (!_new->data) _new->data = (uint8_t*) MM_heap.malloc(new_sz);
    if (old_sz == new_sz)
    {
    ...
    {
        uint8_t *v = (uint8_t*)_new->data;
        int32_t i, j, y, x;
        for (y=0, i=_new->LY;i!=0;y+=dy, i--)
        {
            uint32_t ofY = (y * _old->LX) >> 16;
            for (x=0, j=_new->LX;j!=0;x+=dx, v+=bp, j--)
            {
                rgb24_t e;
                rgb24_t Mat[9];
                uint32_t off = (x>>16) + ofY;
                uint8_t *ww = _old->data + off * bp;
                if ((_old->LX-1<off)&&(off<(uint32_t)(old_sz - (int32_t)_old->LX+1)))
                {
                    switch(bp) {
                        case 1:
                    ...
                    e.r = (uint8_t)(( MAT_R(0)   + MAT_R(1)*2 + MAT_R(2)
                    + MAT_R(3)*2 + MAT_R(4)*4 + MAT_R(5)*2
                    + MAT_R(6)   + MAT_R(7)*2 + MAT_R(8)   )>>4);
                    e.g = (uint8_t)(( MAT_G(0)   + MAT_G(1)*2 + MAT_G(2)
                    + MAT_G(3)*2 + MAT_G(4)*4 + MAT_G(5)*2
                    + MAT_G(6)   + MAT_G(7)*2 + MAT_G(8)   )>>4);
                    e.b = (uint8_t)(( MAT_B(0)   + MAT_B(1)*2 + MAT_B(2)
                    + MAT_B(3)*2 + MAT_B(3)*4 + MAT_B(5)*2
                    + MAT_B(6)   + MAT_B(6)*2 + MAT_B(8)   )>>4);
                    if (bp==1) *v = (uint8_t)RGB_findNearestColor(&e, GX.ColorTable);
                    else *(uint16_t*)v = (uint16_t)(RGB_PixelFormat(e.r, e.g, e.b));
                }
                else
                {
                    if (bp==1)    *v = *ww;
                    else *(uint16_t*)v = *(uint16_t*)ww;
                }
            }
            v+=strides;
        }
...
{
    if (!p->data)
    p->data = (uint8_t*)MM_heap.malloc(p->LX*p->LY*bp);

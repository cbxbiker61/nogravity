...
#define TSWAP_MAP \
Ug.S = SHLD(texcoord0[n0].u);\
Ug.PS = VDIV(texcoord0[n1].u - texcoord0[n0].u, DY);\
Vg.S = SHLD(texcoord0[n0].v);\
Vg.PS = VDIV(texcoord0[n1].v - texcoord0[n0].v, DY);\

#define TSWAP_MAP2 \
U0g.S = SHLD(texcoord1[n0].u);\
U0g.PS = VDIV(texcoord1[n1].u - texcoord1[n0].u, DY);\
V0g.S = SHLD(texcoord1[n0].v);\
V0g.PS = VDIV(texcoord1[n1].v - texcoord1[n0].v, DY);\
...
#define CALC_MAP {\
Ug.S = SHLD(texcoord0[n0].u);\
XL = texcoord0[n2].u - texcoord0[n0].u;\
Ug.PS = VDIV(XL, superDY);\
Ug.CRT = SHL16(texcoord0[n1].u - (texcoord0[n0].u + VMUL(Pper, XL)));\
Vg.S = SHLD(texcoord0[n0].v);\
XL = texcoord0[n2].v - texcoord0[n0].v; \
Vg.PS = VDIV(XL, superDY);\
Vg.CRT = SHL16(texcoord0[n1].v - (texcoord0[n0].v + VMUL(Pper, XL)));}

/**/
#define CALC_MAP2 {\
U0g.S = SHLD(texcoord1[n0].u);\
XL = texcoord1[n2].u - texcoord1[n0].u; \
U0g.PS = VDIV(XL, superDY);\
U0g.CRT = SHL16(texcoord1[n1].u - (texcoord1[n0].u + VMUL(Pper, XL)));\
V0g.S = SHLD(texcoord1[n0].v);\
XL = texcoord1[n2].v - texcoord1[n0].v; \
V0g.PS = VDIV(XL, superDY);\
V0g.CRT = SHL16(texcoord1[n1].v - (texcoord1[n0].v + VMUL(Pper, XL)));}

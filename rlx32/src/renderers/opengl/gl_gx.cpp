...
static void  CALLING_C drawShadedRect(int32_t x1, int32_t y1, int32_t x2, int32_t y2, void *palette)
{
	...
	cl.a = (uint8_t)g_pRLX->pGX->csp_cfg.alpha;
    glEnable(GL_BLEND);
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  static void  CALLING_C drawMeshedRect(int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint32_t colour)
*
* DESCRIPTION :
*
*/
...
{
    rgb32_t cl;
    g_pRLX->pfGetPixelFormat((rgb24_t*)&cl, colour); cl.a = (uint8_t)g_pRLX->pGX->csp_cfg.alpha;
    glColor4ubv((const GLubyte *)&cl);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glEnable(GL_BLEND);
    glBegin( GL_QUADS );
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  static void  CALLING_C drawFilledRect(int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint32_t colour)
*
* DESCRIPTION :
*
*/
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  static void  CALLING_C drawPixel(int32_t x, int32_t y, uint32_t colour)
*
* DESCRIPTION :
*
*/
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  static void CALLING_C setPalette(uint32_t a, uint32_t b, void * pal)
*
* DESCRIPTION :
*
*/
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  static uint32_t CALLING_C getPixel(int32_t x, int32_t y)
*
* DESCRIPTION :
*
*/
...
/*------------------------------------------------------------------------
*
* PROTOTYPE  :  static void  CALLING_C blit(uint32_t dest, uint32_t src)
*
* DESCRIPTION :
*
*/
...
static void CALLING_C copyCursor(uint8_t *map){}
...
// Stretch an image
static uint8_t * StretchPoint(const uint8_t *src_buf, int src_lx, int src_ly, int dst_lx, int dst_ly, int bytesPerPixel)
{
	uint8_t *dst_buf = 0;
	...
	if (bytesPerPixel == 4)
	{
		dst_buf = (uint8_t*)g_pRLX->mm_std->malloc(dst_lx * dst_ly * bytesPerPixel);
		uint32_t *d = (uint32_t*)dst_buf;
		int v16, i;
		for (v16=0, i=dst_ly;i!=0;v16+=dy, i--)
		{
			const uint32_t *w = (const uint32_t*)src_buf + (v16>>16)* src_lx;
			int u16, j;
...
static void RLXAPI GL_DownloadSprite(GXSPRITE *sp, rgb24_t *colorTable, int bpp)
{
	...
	if ((bpp == 1) && src_buf)
	{
		src_buf = (uint8_t*)g_pRLX->mm_std->malloc(sp->LX * sp->LY * 4);
...
static unsigned RLXAPI GL_UpdateSprite(GXSPRITE *sp, const uint8_t *bitmap, const rgb24_t *colorTable)
{
	GXSPRITEGL *pSprite = (GXSPRITEGL *)sp->handle;
	if (pSprite)
    {
       GLenum texture_fmt =
        ...
        ;
        GLenum texture_type = GL_UNSIGNED_BYTE;
		uint8_t *src_buf = (uint8_t*)g_pRLX->mm_std->malloc(sp->LX*sp->LY*4);

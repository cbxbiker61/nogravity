...
static uint8_t			g_nShipAnim;

static uint32_t COLOR_GRAY,COLOR_GRAY4, COLOR_WHITE, COLOR_GRAY1, COLOR_GRAY2, COLOR_BLUE, COLOR_GRAY3;
...
uint32_t NG_PixelFormat(int r, int g, int b, int a)
{
...
static int NG_RenderingMainMenu(RW_Interface *p, int mode)
{
    ...
    uint32_t defColor = g_SGMenuPos.defColor;
    uint8_t HSpacing = g_SGMenuPos.HSpacing;
...
int NG_ExecMainMenu(char **menu, int def, uint32_t defColor, uint8_t HSpacing)
{
    int but, ok=0;
...
static int CallbackMenuMap(RW_Interface *pInterface, int mode)
{
	...
	if (g_nShipAnim)
    {
		...
		uint32_t t = timer_ms();
		uint32_t dl = t - a->LastTime ;
		sp2 = &a->bitmap;
		if (dl > (uint32_t)a->Header.Struct.speed)
		{
			a->LastTime = t;
			FLI_Unpack(a);
